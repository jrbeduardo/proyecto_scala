error id: 
file://<WORKSPACE>/src/main/scala/org/ntic/flights/data/FlightsFileReport.scala
...
object FlightsFileReport {
  ...
  def fromRows(rows: Seq[Try[Row]]): FlightsFileReport = {
    ...
    val validRows = rows.collect { case scala.util.Success(row) => row }
    val invalidRows = rows.collect { case scala.util.Failure(ex) => ex.getMessage }
